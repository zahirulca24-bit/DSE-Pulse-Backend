...
class OhlcRepository:
    ...
    def save(self, result: CsvParseResult) -> None:
        """Atomically replace local OHLC storage with validated rows."""
    ...
    def merge_and_save(
        ...
        """Merge one validated batch without allowing concurrent lost updates."""

        if not uploaded.valid_rows:
            raise ValueError("Cannot merge an OHLC batch with no valid rows.")

        with self._lock:
            existing = self._read_result_unlocked()
            existing_rows = (
                [] if existing is None or not existing.ok else existing.valid_rows
            )
            existing_by_key = {
                (row.symbol.upper(), row.trade_date): row for row in existing_rows
            }
            uploaded_by_key = {
    ...
    def read_result(self) -> CsvParseResult | None:
        with self._lock:
            return self._read_result_unlocked()
    ...
    def get_status(self) -> DataStatusResponse:
        ...
        return DataStatusResponse(
            ...
            rows_count=len(result.valid_rows),
            latest_trade_date=result.latest_trade_date,
            earliest_trade_date=result.earliest_trade_date,
    ...
    def _save_unlocked(self, result: CsvParseResult) -> None:
        ...
        try:
            with NamedTemporaryFile(
                ...
                writer = csv.DictWriter(
                    handle,
                    fieldnames=list(NORMALIZED_HEADERS),
                )
                writer.writeheader()
                for row in result.valid_rows:
    ...
    def _read_result_unlocked(self) -> CsvParseResult | None:
    ...
    def _row_to_csv(row: OhlcRow) -> dict[str, str]:
        return {
            ...
            "open": OhlcRepository._format_number(row.open),
            "high": OhlcRepository._format_number(row.high),
            "low": OhlcRepository._format_number(row.low),
            "close": OhlcRepository._format_number(row.close),
            "volume": str(row.volume),
            "trade": (
                ""
                if row.trade is None
                else OhlcRepository._format_number(row.trade)
            ),
            "value": (
                ""
                if row.value is None
                else OhlcRepository._format_number(row.value)
            ),
        }
