...
class OhlcRepository:
    """Persist and query validated OHLC rows using one local CSV file."""
    ...
    def save(self, result: CsvParseResult) -> None:
        """Atomically save only normalized valid rows from a successful parse."""
    ...
    def read_result(self) -> CsvParseResult | None:
        """Parse the current stored file, or return None when it is absent."""
    ...
    def get_status(self) -> DataStatusResponse:
        """Calculate availability and counts from the current stored file only."""
    ...
    def get_symbols(self) -> SymbolsResponse:
        """Return sorted unique symbols from local valid rows only."""
    ...
    def get_ohlc(
        ...
        """Return date-filtered rows sorted newest first for a case-insensitive symbol."""
    ...
    def _row_to_csv(row: OhlcRow) -> dict[str, str]:
        return {
            ...
            "trade": "" if row.trade is None else OhlcRepository._format_number(row.trade),
