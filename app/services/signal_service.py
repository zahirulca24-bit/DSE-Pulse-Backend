"""Demo fallback and latest local scanner signal service."""
...
class SignalService:
    def __init__(self, scanner_repository: ScannerRepository) -> None:
    ...
    def get_signals(self) -> SignalsResponse:
        ...
        if latest is not None and latest.ok:
            ...
            return SignalsResponse(mode="local_csv", data_source="local_csv", signals=candidates,
    ...
    def get_demo_signals() -> SignalsResponse:
        ...
        for candidate in DEMO_CANDIDATES:
            ...
            signals.append(SignalItem(symbol=candidate.symbol, company=candidate.company,
                sector=candidate.sector, grade=grade, score=candidate.score, signal_status=status,
                entry_status=entry_status, risk_reward=candidate.risk_reward,
                reasons=["Demo technical setup only"], warnings=[], data_mode="Demo Data"))
